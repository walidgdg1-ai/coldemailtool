...
def run(tender_csv,award_csv,out,start,end):
    ...
    raw_tender_rows=len(t); raw_award_rows=len(aw)
    ...
    for d in (t,aw):
        d['_sol']=d['solicitationNumber-numeroSollicitation'].map(clean); d['_ref']=d['referenceNumber-numeroReference'].map(clean); d['_buyer']=d['contractingEntityName-nomEntitContractante-eng'].map(clean); d['_buyer']=d['_buyer'].where(d['_buyer'].notna(),d['contractingEntityName-nomEntitContractante-fra'].map(clean)); d['_key']=d['_sol'].where(d['_sol'].notna(),d['_ref']); fallback=pd.Series(d.index.map(lambda x:'row:'+str(x)),index=d.index); d['_key']=d['_key'].fillna(fallback)+'|'+d['_buyer'].fillna(UNKNOWN).map(norm)
