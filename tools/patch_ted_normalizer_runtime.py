...
new_awards="""    awards=[]
    ...
    bridge_rows_sqlite=c.execute('SELECT award_id,supplier_id,supplier_name,supplier_country,allocated_value FROM bridges').fetchall()
    bridge_by_award=defaultdict(list)
    for br in bridge_rows_sqlite:bridge_by_award[br[0]].append(br[1:])
...
"""
new="""    award_ids={x['Award_ID'] for x in awards};tender_ids={x['Historical_Tender_ID'] for x in tenders};award_supplier_count={x['Award_ID']:x['Supplier_Count'] for x in awards}
    bridges=[];suppliers={}
    for aid,sid,sname,scountry,alloc in bridge_rows_sqlite:
...
new2="""'award_tender_fk':all(x['Historical_Tender_ID'] in tender_ids for x in awards),'multi_supplier_values_not_allocated':all(x['Award_Value_Allocated'] is None for x in bridges if award_supplier_count.get(x['Award_ID']) not in (1,'1'))"""
