...
def role_party(parties,role):
    return [p for p in parties if role in (p.get('roles') or [])]
...
def main():
    ...
    tids={x['Historical_Tender_ID'] for x in tenders};aids={x['Award_ID'] for x in awards};sids=set(suppliers)
