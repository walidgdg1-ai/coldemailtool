...
# Awards do not expose a Supplier_Count column. Derive multi-supplier award membership
# from the authoritative award-supplier bridge instead of referencing a non-existent field.
old_multi="set(adf.loc[adf.Supplier_Count.fillna(0)>1,'Award_ID'])"
new_multi="set(bdf.groupby('Award_ID')['Supplier_ID'].nunique().loc[lambda x:x>1].index)"
if old_multi not in s:
    raise SystemExit('multi-supplier QA target not found; refusing unsafe patch')
...
print('Germany runtime patch applied: date normalization + O(1) buyers + safe QA + bridge-derived multi-supplier check')
