...
def main():
    ...
    con.execute(f"""COPY (
      ...
      SELECT *,CASE WHEN Tender_Count>=20 THEN 'VERY_HIGH_REPEAT' WHEN Tender_Count>=10 THEN 'HIGH_REPEAT' WHEN Tender_Count>=5 THEN 'REPEAT' ELSE 'LOW_REPEAT' END Repeat_Band,
             'NOTICE_FIRST' Evidence_Type,'DERIVED_FROM_GLOBAL_CORE_V3' Derived_Status
    ...
    con.execute(f"""COPY (
      ...
      SELECT *,round(100*(0.35*Vol_Pct+0.20*BuyerBreadth_Pct+0.20*Lean_Pct+0.15*Fragmentation_Pct+
             ...
             'DERIVED_HEURISTIC_V3_NOTICE_FIRST' Derived_Status
    ...
    # USA is intentionally copied as a separately labelled award-first evidence lane, never unioned into notice opportunity counts.
    ...
    quality={'version':'GLOBAL_MARKET_INTELLIGENCE_V3','created_at':created,'source_core_release':'tender-normalized-global-core-v3','notice_first_tenders_expected':1260252,'usa_award_first_source_release':'tender-normalized-usa-awards-v1','outputs':counts,'guardrails':['Notice-first opportunity ranks exclude USA award-first reconstructed rows','No FX mixing','Competition component neutral when bidder coverage <30%','Consortium supplier links fractionalized','All scores labelled DERIVED'],'status':'PASS'}
